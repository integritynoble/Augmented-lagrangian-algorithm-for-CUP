function [U, out] = ftvcs_alp_yang2(A,b,p,q,opts)
%
...
if ~isa(A,'function_handle')
    A = @(u,mode) f_handleA_yang(A,u,mode);
...
sigmay = zeros(p,q);
delta = zeros(size(b,1),size(b,2));                % delta: m
...
for ii = 1:opts.maxit
    ...
    if ~isempty(gp)
        ...
        tau = abs((d'*d)/(dDd + muDbeta*sum(sum(Ad.*Ad))));
